  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row gap-8">
        ...
        <JobFilters filters={filters} onFilterChange={handleFilterChange} onClearFilters={clearAllFilters} />
        </div>
        <div className="flex-1">
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-2">
              <Select value={filters.limit} onValueChange={(value) => handleLimitChange(Number(value))}>
                <SelectTrigger className="w-[90px]">
                  <SelectValue placeholder="Show" value={jobs.length} />
                </SelectTrigger>
                <SelectContent>
              ...
              <Select
                ...
                onValueChange={(value) => handleSortChange(filters.sortField, value)}
              >
                <SelectTrigger className="w-[140px]">
                  <SelectValue placeholder="Sort by" />
                </SelectTrigger>
          ...
          {isLoading ? (
            <div className={viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 gap-4" : "space-y-4"}>
              {[...Array(6)].map((_, index) => (
                <Skeleton key={index} className="h-24 w-full rounded-md bg-gray-200" />
              ))}
            </div>
          ) : jobs.length === 0 ? (
            <div className="flex justify-center items-center h-60">
              <div className="text-center p-6 border border-gray-200 rounded-lg shadow-sm bg-white">
                <h2 className="text-lg font-semibold text-gray-700">No jobs available</h2>
